    ...
    return new Promise((resolve) => {
        ...
        chrome.storage.sync.get("globallyDisabled", function (data) {
            ...
            resolve({cancel: false});
                ...
                console.error("Error in blockUrlCallback", error);
                resolve({cancel: false});
            }
        });
    });
...
function doTheMagic(tabId, frameId, anotherTry) {
    chrome.storage.sync.get("globallyDisabled", function (data) {
        if (data.globallyDisabled) {
            console.log("Extension is disabled, skipping magic");
                    ...
                    if (request.global) {
                        ...
                        if (isDisabled) {
                            ...
                            Object.keys(tabList).forEach(tabId => {
                                setDisabledBadge(parseInt(tabId));
                            });
                        } else {
                        ...
                        // Speichere globalen Status
                        chrome.storage.sync.set({globallyDisabled: isDisabled});
                    }
